...
public class Product {
    ...
    public static ObservableList<Product> getAllProductsadmin(){
        String selectProducts = "SELECT * FROM orders";
        return getProductListadmin(selectProducts);
    }
...
//                );
    ...
    private static ObservableList<Product> getProductListadmin(String query){
        DatabaseConnection dbCon = new DatabaseConnection();
        ObservableList<Product> data1 = FXCollections.observableArrayList();
        try{
            ResultSet rs =  dbCon.getQueryTable(query);
            while(rs.next()){
                data1.add(new Product(rs.getInt("quantity"), rs.getString("customer_id"), rs.getDouble("product_id")));
//                System.out.println(rs.getInt("pid") + " " +
//                        rs.getString("name") + " " +
//                        rs.getDouble("price")
//                );
